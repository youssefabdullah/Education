...
const Sec1 = () => {
    // const [TotalDate, setTotalDate] = useState(false);
    ...
    useEffect(() => {
        ...
        fire.auth().onAuthStateChanged(function (user) {
            if (user) {
                ...
                setId(user.uid)
                console.log(Id)
                db.collection('Sec1/Ex/2').orderBy("score", "desc").onSnapshot(snapshot => {
                    const Student = [];
    ...
    // console.log(moment(new Date(Date1.date)).fromNow())
    useEffect(() => {
        db.collection('Sec1/Ex/1')
            .get().then(snapshot => {
                const Flat = [];
                snapshot.forEach(doc => {
                    const data = doc.data();
    ...
    // var xx = moment(new Date(Date1.date)).fromNow()
    // if(moment(new Date(Date1.date)).fromNow() === "in a few seconds"){
    //     setTotalDate(true)
    // }
    return (
        <div >
